...
void filterCallbackList(CallbackList* list, int (*pred)(Trigger*, void*), void* data)
{
     if(list->first != NULL)
     {
...
void onKeyHelper(lua_State* luaState, int type)
{
...
int onKeyDown(lua_State* luaState)
{
     onKeyHelper(luaState, KeyPressed);
     return 0;
}

int onKeyUp(lua_State* luaState)
{
     onKeyHelper(luaState, KeyReleased);
     return 0;
}

int releaseKey(lua_State *luaState)
{
     int mods = luaL_checkinteger(luaState, 1);
     char const* keyname = luaL_checkstring(luaState, 2);
     int keycode = XKeysymToKeycode(dpy, XStringToKeysym(keyname));
     ...
     trigger.type = KeyRelease;
     removeCallback(callbacks, trigger);
     
     return 0;
}

int releaseAllKeys(lua_State *luaState)
{
